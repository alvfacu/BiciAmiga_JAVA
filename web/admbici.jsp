...
<html lang="en">
  ...
  <body>
    ...
    <div class="col-lg-10 col-centered well">
      <button class="tablink" onclick="openPage('tipos', this)" id="defaultOpen">Tipos de Bicicletas</button>
        ...
        <table class="table" id="tiposbici">
          ...
          <tbody>            
            <% DecimalFormat df2 = new DecimalFormat("0.00");
              ArrayList<TiposBicicleta> tipos = new ControladorTiposBicicletas().getTiposBicicleta();
              if(tipos.size()>0)
      ...
      <div id="bicis" class="tabcontent">
        ...
        <table class="table" id="bicicletas">
          ...
          <tbody>            
            <%ArrayList<Bicicletas> hola = new ControladorBicicletas().getBicicletas();
              if(hola.size()>0){ 
              for (Bicicletas b : hola){%>
                    <tr align="center" >
              ...
              <td style="vertical-align:middle"><%= b.isDisponible() %></td>
              ...
              <td style="vertical-align:middle">
                <button class="btn btn-editar" data-title="Editar" data-toggle="modal" data-target="#edit" 
                        onclick="editarbici('<%= b.getPatente() %>', '<%= b.getTipo().getNombre() %>', '<%= b.getDescripcion() %>', '<%= b.isDisponible() %>', '<%= df2.format(b.getKmDsdMantenimiento()) %>', '<%= df2.format(b.getKmEnViaje()) %>')">
                  <span class="fa fa-edit"></span>
                </button>
              </td>
              <td style="vertical-align:middle">
                <button class="btn btn-eliminar" data-title="Eliminar" data-toggle="modal" data-target="#delete" 
                        onclick="eliminarbici('<%= b.getPatente() %>', '<%= b.getTipo().getNombre() %>', '<%= b.getDescripcion() %>', '<%= b.isDisponible() %>', '<%= df2.format(b.getKmDsdMantenimiento()) %>', '<%= df2.format(b.getKmEnViaje()) %>')">
                  <span class="fa fa-trash-o"></span>
                </button>
              </td>
            </tr>        
            ...
            <div class="col-sm-12">
              <div class="row">
                <div class="col-sm-4 form-group" title="Seleccione Tipo de Bicicleta">
                  <select class="form-control" name="clr">
